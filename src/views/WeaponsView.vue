...
const { skins, prices, getAllSkinsByType, getAllSkinsByName } = jsonCompose();
const gunsToGet = ref([]);

onMounted(() => {
  gunsToGet.value = gunsNamesByType[route.params.type] || [];
});

const weaponsByType = computed(() => {
  gunsToGet.value = gunsNamesByType[route.params.type] || [];
  getAllSkinsByType(gunsToGet.value);
  return gunsToGet.value;
});
